...
{
    /**
     * Sync today's attendance records and report progress via callback.
     *
    ...
    {
        $endpoint = $this->resolveTodayEndpoint($device);
        ...
        if (! $response->successful()) {
            return [
                ...
                'skipped' => 0,
                'failed' => 0,
                'message' => 'Today attendance API request failed with status '.$response->status().'.',
            ];
        }
        ...
        if ($records === null) {
            return [
                ...
                'skipped' => 0,
                'failed' => 0,
                'message' => 'Invalid API payload format for today attendance data.',
            ];
        }
        ...
        foreach (array_chunk($records, 500) as $chunkIndex => $chunk) {
            ...
            foreach ($chunk as $record) {
                try {
                    ...
                    Log::warning('Today attendance import record parse failed.', [
                        'device_id' => $device->id,
                        'record' => $record,
                        'error' => $e->getMessage(),
                    ]);
        ...
        return [
            ...
            'skipped' => $skipped,
            'failed' => $failed,
            'message' => 'Today import completed successfully.',
        ];
    }

    protected function resolveTodayEndpoint(Device $device): string
    {
        $baseUrl = $this->normalizeBaseUrl((string) ($device->api_url ?: config('services.device_gateway.url', 'http://localhost:4370')));
        $path = (string) parse_url($baseUrl, PHP_URL_PATH);

        if (Str::contains(Str::lower($path), '/api/attendance/today')) {
            return rtrim($baseUrl, '/');
        }

        return rtrim($baseUrl, '/').'/api/attendance/today';
    }
